function [Ref] = Coeff_Reflex(obj,theta_i)
% COEFF_REFLEX Summary of this function goes here
% This function simply returns the reflection coefficient.
...
Ref = Tref_orthogonal - (1 - Tref_orthogonal^2)*Tref_orthogonal*exp(-2*gamma*s)*exp(2*1j*b*s*sin(theta_t)*sin(theta_i))/(1 - Tref_orthogonal^2*exp(-2*gamma*s)*exp(1j*b*2*s*sin(theta_t)*sin(theta_i)));
